...
WORDPRESSID=$(curl -s -X GET "https://astra.demo.netapp.com/accounts/$ACCOUNTID/k8s/v2/apps?include=id&filter=name%20eq%20%27wpbr%27" -H "Authorization: Bearer $APITOKEN" | jq -r .items[0][0])
...
RESTOREAPP=$(curl -k -s -X POST "https://astra.demo.netapp.com/accounts/$ACCOUNTID/k8s/v2/apps" \
  ...
  -d @CURL-ACC-wpbrhook-restore-app.json)
